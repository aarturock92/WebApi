...
namespace CEMEX.API.Infrastructure.Validators.Catalogos
{
    public class MovilViewModelValidator: AbstractValidator<MovilViewModel>
    {
        ...
        {
            RuleFor(m => m.IdRegion).NotEmpty().WithMessage("El campo Región es requerido")
                                    .Must(m => m > 0).WithMessage("El campo Región es requerido");

            RuleFor(m => m.IdPlazaImmex).NotEmpty().WithMessage("El campo Plaza Immex es requerido")
                                        .Must(m => m > 0).WithMessage("El campo Plaza Immex es requerido");
